    ...
    return (
        <div className="container mx-auto px-4 py-8 max-w-5xl z-10 relative">
            <header className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
                <div>
                    <h1 className="font-outfit text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-amber-400 to-rose-400 flex items-center gap-2">
                        <span>⚙️</span> 系統權限管理
                    </h1>
                    <p className="text-slate-400 mt-1 text-sm">超級管理者模式：管控全域發起會議權限</p>
                </div>
            ...
            <div className="mb-6 bg-white/5 border border-white/10 rounded-xl p-3 max-w-md flex items-center gap-3">
                ...
                <input
                    ...
                    className="bg-transparent border-none text-white w-full outline-none text-sm placeholder-slate-500"
                />
            </div>
            ...
            >
                <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-white/5 border-b border-white/10">
                                <th className="p-5 text-slate-400 text-xs font-semibold uppercase tracking-wider">使用者</th>
                                <th className="p-5 text-slate-400 text-xs font-semibold uppercase tracking-wider">單位</th>
                                <th className="p-5 text-slate-400 text-xs font-semibold uppercase tracking-wider">全域角色</th>
                                <th className="p-5 text-slate-400 text-xs font-semibold uppercase tracking-wider text-center">授權發起會議</th>
                            </tr>
                        </thead>
                        <tbody>
                            {filteredUsers.map(u => {
                                    ...
                                    <tr key={u.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                                        <td className="p-5">
                                            <div className="flex items-center gap-3">
                                                <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-bold text-white shadow-inner ${isSelf ? 'bg-rose-500' : 'bg-indigo-500'}`}>
                                        ...
                                        <td className="p-5 text-sm text-slate-300">{u.dept}</td>
                                        <td className="p-5">
                                            {u.role === 'super_admin' && <span className="text-amber-400 font-bold text-sm">超級管理者</span>}
                                            {u.role === 'creator' && <span className="text-emerald-400 text-sm">授權發起人</span>}
                                            {u.role === 'user' && <span className="text-slate-400 text-sm">一般使用者</span>}
                                        </td>
                                        <td className="p-5 text-center">
                                            {isSelf ? (
                                                <span className="text-xs text-slate-500">(系統預設權限)</span>
                                            ) : (
                    ...
                    {filteredUsers.length === 0 && (
                        <div className="p-8 text-center text-slate-400">找不到符合的使用者</div>
                    )}
                </div>
            </motion.div>
        </div>
